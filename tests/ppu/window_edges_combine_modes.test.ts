...
describe('Window edges for combine modes (AND/XOR/XNOR)', () => {
  ...
  it('AND combine includes overlap boundary (x=3) when A=[2..3], B=[3..4]', () => {
    ...
    const ppu = setup(bus);
    // A [2..3], B [3..4]
    w8(bus, mmio(0x26), 0x02); w8(bus, mmio(0x27), 0x03);
    ...
    expect(px(3)[0]).toBeGreaterThan(100); expect(px(3)[1]).toBeGreaterThan(100); // boundary included
    expect(px(2)[0]).toBeGreaterThan(200); expect(px(2)[1]).toBeLessThan(10);
    expect(px(4)[0]).toBeGreaterThan(200); expect(px(4)[1]).toBeLessThan(10);
  });
  ...
  it('XOR combine excludes overlap boundary (x=4) when A=[2..4], B=[4..6]', () => {
    ...
    const ppu = setup(bus);
    // A [2..4], B [4..6]
    w8(bus, mmio(0x26), 0x02); w8(bus, mmio(0x27), 0x04);
    w8(bus, mmio(0x28), 0x04); w8(bus, mmio(0x29), 0x06);
