import { describe, it, expect } from 'vitest';
import { SNESBus } from '../../src/bus/snesBus';
import { Cartridge } from '../../src/cart/cartridge';
import { renderMainScreenRGBA } from '../../src/ppu/bg';
const mmio = (reg) => (0x00 << 16) | (0x2100 + (reg & 0xff));
const w8 = (bus, addr, v) => bus.write8(addr, v);
function mkBus() {
    const rom = new Uint8Array(0x20000);
    const cart = new Cartridge({ rom, mapping: 'lorom' });
    return new SNESBus(cart);
}
describe('Mixed main/sub combine modes (main uses A, sub uses B)', () => {
    function writeSolid(bus) {
        for (let y = 0; y < 8; y++) {
            w8(bus, mmio(0x16), (0x1000 + 16 + y) & 0xff);
            w8(bus, mmio(0x17), ((0x1000 + 16 + y) >>> 8) & 0xff);
            w8(bus, mmio(0x18), 0xff);
            w8(bus, mmio(0x19), 0x00);
            w8(bus, mmio(0x16), (0x1000 + 16 + 8 + y) & 0xff);
            w8(bus, mmio(0x17), ((0x1000 + 16 + 8 + y) >>> 8) & 0xff);
            w8(bus, mmio(0x18), 0x00);
            w8(bus, mmio(0x19), 0x00);
        }
    }
    function setup(bus) {
        const ppu = bus.getPPU();
        w8(bus, mmio(0x00), 0x0f);
        w8(bus, mmio(0x2c), 0x01); // BG1 main
        w8(bus, mmio(0x2d), 0x02); // BG2 sub
        w8(bus, mmio(0x0b), 0x22);
        w8(bus, mmio(0x08), 0x04);
        writeSolid(bus);
        // BG1 red, BG2 green
        w8(bus, mmio(0x16), 0x00);
        w8(bus, mmio(0x17), 0x00);
        w8(bus, mmio(0x18), 0x01);
        w8(bus, mmio(0x19), 0x00);
        w8(bus, mmio(0x16), 0x00);
        w8(bus, mmio(0x17), 0x02);
        w8(bus, mmio(0x18), 0x01);
        w8(bus, mmio(0x19), 0x04);
        w8(bus, mmio(0x21), 2);
        w8(bus, mmio(0x22), 0x00);
        w8(bus, mmio(0x22), 0x7c);
        w8(bus, mmio(0x21), 34);
        w8(bus, mmio(0x22), 0xe0);
        w8(bus, mmio(0x22), 0x03);
        // Color math add-half, mask=BG1; subscreen gating on
        w8(bus, mmio(0x31), 0x60 | 0x01);
        return ppu;
    }
    it('AND combine: blend only where A and B overlap', () => {
        const bus = mkBus();
        const ppu = setup(bus);
        // A [1..2], B [2..3]
        w8(bus, mmio(0x26), 0x01);
        w8(bus, mmio(0x27), 0x02);
        w8(bus, mmio(0x28), 0x02);
        w8(bus, mmio(0x29), 0x03);
        // Enable BG1 A and B (bits0|1) and BG2 B only (bit3)
        w8(bus, mmio(0x23), 0x03 | 0x08);
        // applyInside=1, AND combine, subscreen gate on
        w8(bus, mmio(0x30), 0x03 | (1 << 6));
        const rgba = renderMainScreenRGBA(ppu, 6, 1);
        const px = (x) => [rgba[x * 4], rgba[x * 4 + 1]];
        // Blend only at x=2
        expect(px(2)[0]).toBeGreaterThan(100);
        expect(px(2)[1]).toBeGreaterThan(100);
        // x=1 main in A but sub masked -> half red
        expect(px(1)[0]).toBeGreaterThan(100);
        expect(px(1)[1]).toBeLessThan(10);
        // x=3 sub in B but main outside A -> no math (pure red)
        expect(px(3)[0]).toBeGreaterThan(200);
        expect(px(3)[1]).toBeLessThan(10);
    });
    it('XOR combine: blend where exactly one of A or B is active', () => {
        const bus = mkBus();
        const ppu = setup(bus);
        // A [0..2], B [2..4]
        w8(bus, mmio(0x26), 0x00);
        w8(bus, mmio(0x27), 0x02);
        w8(bus, mmio(0x28), 0x02);
        w8(bus, mmio(0x29), 0x04);
        // BG1 A and B enabled for main combine; BG2 B only for subscreen
        w8(bus, mmio(0x23), 0x03 | 0x08);
        // applyInside=1, XOR, subscreen gate on
        w8(bus, mmio(0x30), 0x03 | (2 << 6));
        const rgba = renderMainScreenRGBA(ppu, 6, 1);
        const px = (x) => [rgba[x * 4], rgba[x * 4 + 1]];
        // x=1: main inside, sub masked -> half red
        expect(px(1)[0]).toBeGreaterThan(100);
        expect(px(1)[1]).toBeLessThan(10);
        // x=2: both active (boundary) -> XOR false -> no blend (pure red)
        expect(px(2)[0]).toBeGreaterThan(200);
        expect(px(2)[1]).toBeLessThan(10);
        // x=3: sub inside B only; main XOR A^B -> at x=3 only B -> mainAffected true, so blend
        expect(px(3)[0]).toBeGreaterThan(100);
        expect(px(3)[1]).toBeGreaterThan(100);
    });
});
