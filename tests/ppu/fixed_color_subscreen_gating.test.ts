...
describe('Subscreen fixed color gating with window masks over backdrop sub', () => {
  function setupBG1Red_OBJoff(bus: SNESBus) {
    ...
    w8(bus, mmio(0x21), 0); w8(bus, mmio(0x22), 0x00); w8(bus, mmio(0x22), 0x00);
    return ppu;
  }
  ...
  it('sub gate masks subscreen to backdrop when fixed mode off (applyInside=1)', () => {
    ...
    expect(px(1)[0]).toBeGreaterThan(80); expect(px(1)[0]).toBeLessThan(200);
    // Outside A -> main math still applies with unmasked subscreen=backdrop -> also darker than pure red
    expect(px(5)[0]).toBeGreaterThan(80); expect(px(5)[0]).toBeLessThan(200);
  });
  ...
  it('sub gate masks to fixed color when fixed mode ON', () => {
    ...
    expect(px(5)[0]).toBeGreaterThan(80); expect(px(5)[1]).toBeGreaterThan(20);
  });
});
