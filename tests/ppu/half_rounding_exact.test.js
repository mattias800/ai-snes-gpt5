...
function scale5To8(v) { return Math.floor((v * 255) / 31); }
function writeSolid4bppTile1(bus) {
...
describe('Color math half rounding exactness (add/sub)', () => {
    it('add-half exact channel rounding using fixed color multi-channel', () => {
        ...
        const R = 7, G = 15, B = 23;
        const bgr15 = (R << 10) | (G << 5) | (B);
        w8(bus, mmio(0x21), 2);
        w8(bus, mmio(0x22), bgr15 & 0xff);
        w8(bus, mmio(0x22), (bgr15 >> 8) & 0xff);
