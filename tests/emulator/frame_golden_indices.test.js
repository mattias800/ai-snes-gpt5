...
function pushByte(prog, b) { prog.push(b & 0xff); }
function ldaImm(prog, v) { pushByte(prog, 0xa9); pushByte(prog, v); }
function staAbs(prog, addr) {
    pushByte(prog, 0x8d);
    pushByte(prog, addr & 0xff);
    pushByte(prog, (addr >>> 8) & 0xff);
}
function buildProgram() {
    ...
    for (let y = 0; y < 8; y++) {
        ...
        ldaImm(p, (tileBaseWord + y) & 0xff);
        staAbs(p, 0x2116);
        ldaImm(p, ((tileBaseWord + y) >>> 8) & 0xff);
        staAbs(p, 0x2117);
        ...
        ldaImm(p, (tileBaseWord + 8 + y) & 0xff);
        staAbs(p, 0x2116);
        ldaImm(p, ((tileBaseWord + 8 + y) >>> 8) & 0xff);
        staAbs(p, 0x2117);
        ldaImm(p, 0x00);
        staAbs(p, 0x2118);
        ldaImm(p, 0x00);
    ...
    function writeMapEntry(offsetWords, value) {
        ldaImm(p, (0x0000 + offsetWords) & 0xff);
        staAbs(p, 0x2116);
        ldaImm(p, ((0x0000 + offsetWords) >>> 8) & 0xff);
        staAbs(p, 0x2117);
        ldaImm(p, value & 0xff);
        staAbs(p, 0x2118);
        ldaImm(p, (value >>> 8) & 0xff);
        staAbs(p, 0x2119);
    }
    const pal = (g) => (g & 7) << 10;
