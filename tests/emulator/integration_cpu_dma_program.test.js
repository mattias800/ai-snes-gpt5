...
function buildProgram() {
    const p = [];
    const lda = (v) => { p.push(0xa9, v & 0xff); };
    const sta = (addr) => { p.push(0x8d, addr & 0xff, (addr >>> 8) & 0xff); };
...
describe('Integration: CPU-configured DMA to VRAM and CGRAM then RGBA render', () => {
    it('loads tile and palette via DMA and renders correct colors', () => {
        ...
        for (let i = 16; i < 32; i++)
            bus.write8((0x7e << 16) | (0x1100 + i), 0x00);
