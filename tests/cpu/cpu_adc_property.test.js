...
describe('Property-based: ADC #imm (8-bit, decimal off)', () => {
    it('matches pure model across random inputs', () => {
        fc.assert(fc.property(fc.integer({ min: 0, max: 255 }), fc.integer({ min: 0, max: 255 }), fc.boolean(), (a, imm, carryIn) => {
            ...
            expect(((cpu.state.P & 1 /* Flag.C */) !== 0) ? 1 : 0).toBe(model.C);
            expect(((cpu.state.P & 64 /* Flag.V */) !== 0) ? 1 : 0).toBe(model.V);
            expect(((cpu.state.P & 2 /* Flag.Z */) !== 0) ? 1 : 0).toBe(model.Z);
            expect(((cpu.state.P & 128 /* Flag.N */) !== 0) ? 1 : 0).toBe(model.N);
        }), { numRuns: 200 });
    });
});
