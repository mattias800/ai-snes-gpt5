...
function writeProgram(bus: TestMemoryBus, bank: number, pc: number, bytes: number[]) {
  let a = (bank << 16) | pc;
  for (const b of bytes) bus.write8(a++, b & 0xff);
}

function r8(bus: TestMemoryBus, addr24: number) { return bus.read8(addr24); }
...
describe('CPU width-sensitive memory ops (LDA/STA/CMP abs 16-bit)', () => {
  it('handles 16-bit A with REP M=0 and absolute addressing', () => {
    ...
    for (let i = 0; i < 9; i++) cpu.stepInstruction();
