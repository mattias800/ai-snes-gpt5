...
function writeByte(bus: TestMemoryBus, bank: number, addr: number, value: number) {
  bus.write8(((bank & 0xff) << 16) | (addr & 0xffff), value & 0xff);
}

function writeProg(bus: TestMemoryBus, bank: number, start: number, bytes: number[]) {
  for (let i = 0; i < bytes.length; i++) writeByte(bus, bank, start + i, bytes[i]);
}
